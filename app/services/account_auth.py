...
class AccountAuthManager:
    ...
    def _new_client(self) -> TelegramClient:
        ...
        return TelegramClient(StringSession(), settings.api_id, settings.api_hash, connection_retries=4, request_retries=2)
    ...
    async def start_phone(self, owner_user_id: int, phone: str):
        ...
        try:
            ...
            raise
        self.contexts[owner_user_id] = LoginContext(
    ...
    async def _finish(self, owner_user_id: int) -> ConnectedAccount:
        ...
        raw_session = StringSession.save(ctx.client.session)
    ...
    def mask_phone(phone: str | None) -> str | None:
        ...
        if len(p) <= 4:
            return "••••"
        prefix = "+" + p[:1]
        return prefix + "•" * max(4, len(p) - 5) + p[-4:]
